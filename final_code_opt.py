...
def fit_and_evaluate_model_manual(X_train, X_test, y_train, y_test, dataset_model_scores, log_file=None, training_time=None):
...
def fit_and_evaluate_model_hyperopt(X_train, X_test, y_train, y_test, dataset_model_scores, max_evals=20, log_file=None, training_time=None):
    if training_time is None:
        training_time = defaultdict(float)
...
def fit_and_evaluate_model_gpbo(X_train, X_test, y_train, y_test, acq_func, dataset_model_scores, log_file=None, training_time=None):
    if training_time is None:
        training_time = defaultdict(float)
...
def fit_and_evaluate_model_tuned(X_train, X_test, y_train, y_test, dataset_model_scores, optimization="random", log_file=None, training_time=None):
    if training_time is None:
        training_time = defaultdict(float)
...
def explore_parquet_folder(folder_path, log_file=None, training_time=None, optimization="manual", dataset_model_scores=None):
    ...
    if dataset_model_scores is None:
    ...
    if optimization == "manual":
        fit_and_evaluate_model_manual(X_train, X_test, y_train_enc, y_test_enc, dataset_model_scores, log_file, training_time)
    elif optimization in ("random", "bayesian", "bo"):
        fit_and_evaluate_model_tuned(X_train, X_test, y_train_enc, y_test_enc, dataset_model_scores, optimization, log_file, training_time)
    ...
    for dataset_type in sorted(base.iterdir()):
        ...
        if dataset_type.is_dir():
            ...
            for fold in sorted(dataset_type.iterdir(), key=lambda x: int(x.name)):
                ...
                if fold.is_dir():
                    fold_count += 1
                    print_log(f"\n[Dataset Summary: {dataset_type.name}]", log_file)
                    print_log(f"\nExploring fold: {fold.name}", log_file)
                    explore_parquet_folder(fold, log_file, training_times, optimization=optimization, dataset_model_scores=dataset_model_scores)
                else:
                    print_log(f"  Skipping non-folder in dataset_type: {fold.name}", log_file)
            print_log(f"\n[Dataset Summary: {dataset_type.name} | Optimization: {optimization}]", log_file)
            for model_name in dataset_model_scores:
                ...
                acq_funcs = dataset_model_scores[model_name].get('acq_funcs', None)
                if acq_funcs:
                    for acq_func, score, param in zip(acq_funcs, scores, params):
                        avg_score = np.mean(scores)
                        best_fold = np.argmax(scores)
                        final_best_params = params[best_fold]
                        print_log(f"Model: {model_name} | AcqFunc: {acq_func}", log_file)
