...
def fit_and_evaluate_model_manual(X_train, X_test, y_train, y_test, log_file=None, training_time=None):
    ...
    for model_name, model in models.items():
        ...
        print_log(f"Retraining {model_name} on entire training set (manual)...", log_file)
        start_time = time.time()
        model.fit(X_train, y_train)
...
def fit_and_evaluate_model_hyperopt(X_train, X_test, y_train, y_test, max_evals=20, log_file=None, training_time=None):
    if training_time is None:
        training_time = defaultdict(float)
...
def fit_and_evaluate_model_gpbo(X_train, X_test, y_train, y_test, log_file=None, training_time=None):
    if training_time is None:
        training_time = defaultdict(float)
...
def fit_and_evaluate_model_tuned(X_train, X_test, y_train, y_test, optimization="random", log_file=None, training_time=None):
    if training_time is None:
        training_time = defaultdict(float)
...
def explore_parquet_folder(folder_path, log_file=None, training_time=None, optimization="manual"):
    ...
    if optimization == "manual":
        ...
        for acq_fn in ["EI", "LCB", "PI"]:
            ...
            fit_and_evaluate_model_gpbo(X_train, X_test, y_train_enc, y_test_enc, log_file=log_file, training_time=training_time, acq_func=acq_fn)
    else:
